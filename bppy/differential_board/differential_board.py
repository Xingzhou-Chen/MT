...
i2c1 = I2C(id=0,scl=Pin(9),sda=Pin(8),freq=1000_000)
i2c = I2C(id=1,scl=Pin(7),sda=Pin(6),freq=1000_000)
valve_f = PWM(Pin(15))
valve_f.freq(100000)
valve_b = PWM(Pin(16))
valve_b.freq(100000)
valve_m = Pin(11,Pin.OUT)
pump = Pin(10,Pin.OUT)
...
Target_Pressure=160
P_end=Target_Pressure-120#130/90
...
def pump_up(Target_Pressure):
    ...
    current_pressure_b=read_b(sensor_b,range_b_min,range_b_max)
    # print("b:",current_pressure_f)
    while current_pressure_f< Target_Pressure  or current_pressure_b<Target_Pressure-20:
        pump_on()
        time.sleep(0.2)
        pump_off()
        time.sleep(0.3)
        # print("f2",current_pressure_f)
        current_pressure_f=read(sensor_f,range_f_min,range_f_max)
        current_pressure_b=read_b(sensor_b,range_b_min,range_b_max)

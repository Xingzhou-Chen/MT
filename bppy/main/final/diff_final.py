...
def read(address,pmin,pmax):                                        # reading function of ams sensor modified based on the arduino library of AMS 
...
def calculate_k(P_init,P_end,T_end):                                # compute the slope of setpoint
    k=(P_end-P_init)/T_end
    return k

def calculate_ref(k,P_init,current_t):                              # compute the setpoint of baseline volume
...
def pump_up(Target_Pressure):
    ...
    while current_pressure_f< Target_Pressure-40: # direct pumping 
        pump_on()
    ...
    while current_pressure_f< Target_Pressure or current_pressure_b<Target_Pressure-20: # step-wise pumping
        pump_on()
        time.sleep(0.2)
        pump_off()
        time.sleep(0.3)
    ...
    while current_pressure_f> Target_Pressure+1:                                        # release cuff volume 
        frontvalve_on(25000)
        current_pressure_f=read(sensor_f,range_f_min,range_f_max) 
    frontvalve_off() 

    current_pressure_b=read(sensor_b,range_b_min,range_b_max)
    while current_pressure_b> Target_Pressure-19:                                       # release baseline volume
        backvalve_on(25000)
        current_pressure_b=read(sensor_b,range_b_min,range_b_max) 
    backvalve_off() 
    pump_off()
...
def set_init(current_pressure):                                                         # preserve the initial pressure
...
if __name__== '__main__':   
    ...
    difference=current_pressure_f-current_pressure_b                    
    while current_pressure_f>P_end:                                     # deflation 
        current_pressure_b=read(sensor_b,range_b_min,range_b_max)       #state 2: regulation of Back 
        current_pressure_f=read(sensor_f,range_f_min,range_f_max)
        ...
        current_pressure_m=read(sensor_m,range_m_min,range_m_max)       # read the pressure difference 

...
def PID_control(Kp,Ki,Kd,set_point,current_p):
    ...
    int_err=err+int_err                                                     # accumulate errors
    P=Kp*err
    I=Ki*int_err
    D=Kd*(err-last_err)
    u=P+I+D
...
def regulation(delta,P_ref,current_pressure):
    ...
    release_speed=u+ff
    release_speed=value_calibrate(release_speed,delta)                      # calibrate the computed value before it is applied
...
if __name__== '__main__':   
    ...
    while current_pressure>P_end:                                           # deflation 
        current_pressure=read(sensor_address,range_min,range_max)
        delta = time.ticks_diff(time.ticks_ms(), start)/1000                # current time 

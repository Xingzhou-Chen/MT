...
def pump_up(Target_Pressure):
    ...
    while current_pressure_f< Target_Pressure :
        ...
        if(current_pressure_b>Target_Pressure):midval_off()
    
    while current_pressure_f> current_pressure_b: # release f
        frontvalve_on(29000)
        current_pressure_f=read(sensor_f,range_f_min,range_f_max) 
        current_pressure_b=read(sensor_b,range_b_min,range_b_max)
    frontvalve_off() 
...
def regulation_b_step(P_ref):
    global P_start
    pressure_difference=read(sensor_m,range_m_min,range_m_max)
    difference=0
    if(pressure_difference<20):
        backvalve_off()
        difference=P_start-P_ref
    else:
        time.sleep(0.02)
        pressure_difference=read(sensor_m,range_m_min,range_m_max)
        while pressure_difference>0:
            backvalve_on(int(14000))
            pressure_difference=read(sensor_m,range_m_min,range_m_max)
        backvalve_off()  

        # while pressure_difference>0:
        #     backvalve_on(int(30000))
        #     time.sleep(0.01)
        #     backvalve_off()  
        #     time.sleep(0.05)
        #     pressure_difference=read(sensor_m,range_m_min,range_m_max)
...
if __name__== '__main__':   
    ...
    while current_pressure_f>P_end:                                     # deflation 
        ...
        print(delta,",",current_pressure_f,",",current_pressure_b,",",current_pressure_m,",",d-current_pressure_m)
        # print(delta,",",current_pressure_f,",",current_pressure_m,",",d-current_pressure_m)
    release()                                                           #release
    # print(pwm)
...
def feed_forward_b(t,count):
    if(count<len(tb)): ff=float(tb[count])
    else:ff=34012.94-50.25*t+0.33*t**2-0.01*t**3
    return ff
...
def regulation_b(delta,P_ref_b,current_pressure_b,count):
    ff_b=feed_forward_b(delta,count)
...
def regulation_b_tb(delta,P_ref_b,current_pressure_b,count):
    ff_b=feed_forward_b(delta,count)
    # u_b=PID_control_b(Kp_b,Ki_b,Kd_b,P_ref_b,current_pressure_b)
    # release_speed_b=u_b+ff_b
...
if __name__== '__main__':   
    ...
    while current_pressure_f>P_end:                                     # deflation 
        ...
        release_speed_b=regulation_b(delta,P_ref_b,current_pressure_b,count)  #state 2: regulation of Back 

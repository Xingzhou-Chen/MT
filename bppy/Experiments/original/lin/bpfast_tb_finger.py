...
def regulation_tb(count):
    if(count<1000):release_speed=1
    else:release_speed=0
    # release_speed=value_calibrate(release_speed)
    return release_speed
...
if __name__== '__main__':
    ...
    while current_pressure>P_end:                                   # deflation      
        ...
        # release_speed=regulation_tb(count)
        c.valve_on(int(release_speed))                                #state3: release

        pulse=current_pressure-P_ref+10
        uart.write(str(delta)+" , "+str(pulse)+"\n")
        count=count+1

    c.release()                                                       # final release
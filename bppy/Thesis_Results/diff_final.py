...
# T_end=40
# # Target_Pressure=150
# Target_Pressure=110 #arm
# # P_end=Target_Pressure-120
# P_end=20

T_end=40
# Target_Pressure=180
# P_end=Target_Pressure-140 #150/80

Target_Pressure=160
P_end=Target_Pressure-140#130/90
...
# Kp_f=4000
# Ki_f=110
# Kd_f=0 # sim sysdia dog

Kp_f=2100
Ki_f=5
Kd_f=0 # sim sysdia cat
...
def value_calibrate_f(v,t):
    # if(t>25 and v>1000):return feed_forward_f(t)
    # if(v>feed_forward_f(t)+1500 or v<v>feed_forward_f(t)-2000): return max_pwm_f
    if(v>max_pwm_f or v<min_pwm_f):return feed_forward_f(t)
    # if(err_f>5 or err_f<-5):return feed_forward_f(t)
    else: return v
...
def feed_forward_f(t):
    ...
    # ff=12770.6-216*t#sim
    # ff=-4.761*t**2 - 232*t + 2.459e+04
    # ff=-327.9*t + 1.846e+04
    # ff=-338.4*t + 1.587e+04
    # ff=-0.1696*t**3 + 5.863*t**2 - 393.5*t + 2.61e+04
    # ff=-0.1508*t**3 +21.19*t**2 - 877.3*t + 1.846e+04
    # ff=-0.263*t**3 + 7.284*t**2 - 348.7*t + 2.314e+04#arm
    # ff=-0.1696*t**3 + 5.863*t**2 - 393.5*t + 2.61e+04#arm
    # ff=-332.2*t + 2.014e+04#sim
    ff=-187.7*t + 1.272e+04#sim real
    # ff=-189.4*t + 1.284e+04
    # ff=-195.4*t + 1.967e+04
    # ff=-192.3*t + 1.198e+04
    # ff=-0.1673*t**3 + 5.495*t**2 - 349.1*t + 3.726e+04#finger
    return ff

def feed_forward_b(t):
    # ff=-0.2675*t**3 + 10.26*t**2 - 383.4*t + 4.172e+04
...
if __name__== '__main__':   
    ...
    while current_pressure_f>P_end:                                     # deflation 
        ...
        print(delta,",",current_pressure_m-difference)
        # print(delta,",",release_speed_f)
        # print(delta,",",current_pressure_f,",",current_pressure_b,",",current_pressure_m-difference)
        # print(delta,",",current_pressure_f,",",P_ref_f,",",current_pressure_b,",",P_ref_b,",",current_pressure_m-difference)
        # print(delta,",",current_pressure_f,",",P_ref_f,",",current_pressure_b,",",1,",",current_pressure_f-P_ref_f)
        # print(delta,",",release_speed_f,",",current_pressure_m)
        # pulse_f=current_pressure_f-P_ref_f
        # pulse_f=current_pressure_f-P_ref_f
        # print(delta,",",release_speed_b)
        # print(delta,",",current_pressure_f,",",P_ref_f)
        # print(delta,",",current_pressure_b,",",P_ref_b)
        # pulse_b=current_pressure_b-P_ref_b
        # print(delta,",",pulse_b)
        # print(delta,",",release_speed_f,",",current_pressure_f,",",current_pressure_m-difference)
        # print(delta,",",current_pressure_m-difference)
